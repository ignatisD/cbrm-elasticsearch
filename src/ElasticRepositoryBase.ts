...
export default abstract class ElasticRepositoryBase<T extends IEsDoc = any> extends Repository<IEsModel<T>> implements IRepositoryBase<T> {
    ...
    public get client(): ElasticsearchConnector {
        return this._client;
    }
    ...
    public build(terms: IQuery, builder: bodyBuilder.Bodybuilder = null): any {
    ...
    public paginate(terms: IQuery, hits: IHit<T>[] = [], total: number = 0): IPaginatedResults<T> {
    ...
    async retrieve(terms: IQuery): Promise<JsonResponse<IPaginatedResults<T>>> {
        return await this.search(terms);
    }

    protected async _handleResults(terms: IQuery, res: ApiResponse<ISearchResponse<T>>, body?: any): Promise<JsonResponse<IPaginatedResults<T>>> {
    ...
    async scroll(terms: IQuery): Promise<JsonResponse<IPaginatedResults<T>>> {
    ...
    async * scrollSearch (terms: IQuery): AsyncGenerator<IPaginatedResults<T>, void> {
        if (!terms.scroll) {
            terms.scroll = 20;
        }
    ...
    async find(terms: IQuery): Promise<JsonResponse<IPaginatedResults<T>>> {
        return await this.search(terms);
    }
    ...
    async findById(terms: IQuery): Promise<JsonResponse<T>> {
        try {
            ...
            if (!res.body?.hits) {
                if (response.errors) {
                    Logger.error("findById: " + response.reason());
                }
    ...
    async findOne(terms: IQuery) {
        try {
            ...
            if (!res.body?.hits) {
                if (response.errors) {
                    Logger.error("findOne: " + response.reason());
                }
     ...
     * @param item
     */
    async updateMany(terms: IQuery, item: Partial<T> | any): Promise<JsonResponse<number>> {
        try {
    ...
    async deleteMany(terms: IQuery): Promise<JsonResponse<number>> {
        try {
            const q = terms.raw ?? this.build(terms);
    ...
    async count(terms: IQuery): Promise<JsonResponse<number>> {
        try {
            const q = terms.raw ?? this.build(terms);
            const params: IElasticParams = {
                index: terms.index || this.searchIndex,
    ...
    async aggregate<A = any>(terms: IQuery): Promise<JsonResponse<IAggregationResponse<T, A>>> {
        try {
            const body = terms.raw ?? this.build(terms);
            const params: IElasticParams = {
                index: terms.index || this.searchIndex,
    ...
    async clearScroll(terms: IQuery) {
        try {
            if (!terms.scrollId) {
